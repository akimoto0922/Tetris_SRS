...
public class RotationBattle : MonoBehaviour
{
    ...
    int[,] field_player1 = new int[12, 22];
    ...
    void Update()
    {
        minoDirection = battleManager.GetComponent<BattleManager>().minoDirection;
    }
    // テストの数は4種類
    // ＜右回転＞
    // ・J,L,S,Z,Tミノ
    ...
    {
        // ①値を参照する
        field_player1 = battleManager.GetComponent<BattleManager>().field_player1;
        currentMino = battleManager.GetComponent<BattleManager>().currentMino;
        minoDirection = battleManager.GetComponent<BattleManager>().minoDirection;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            Debug.Log("回転できない；；");
                            minoDirection = 0;
                            battleManager.GetComponent<GameManager>().minoDirection = minoDirection;
                            count = 0;
                            Initialize_All();
                        }
                    }
    ...
    void Copy()
    {
        for (int i = 0; i < field_player1.GetLength(0); i++)
        {
            ...
            {
                // ミノ
                if (field_player1[i, j] == 1 || field_player1[i, j] == 2)
                {
                    minoArray[i, j] = field_player1[i, j];
                }

                // 壁
                if (field_player1[i, j] == 3)
                {
                    fieldArray[i, j] = field_player1[i, j];
                }
            }
        }
    }
    ...
    void Rotate_field()
    {
        ...
        for (int i = 0; i < field_player1.GetLength(0); i++)
        {
            ...
            for (int j = 0; j < field_player1.GetLength(1); j++)
            {
                ...
                if (field_player1[i, j] == 2 || field_player1[i, j] == 1)
                {
                    field_player1[i, j] = 0;
                }
            }
        ...
        for (int i = 0; i < minoArray.GetLength(0); i++)
        {
            ...
            for (int j = 0; j < minoArray.GetLength(1); j++)
            {
                ...
                {
                    field_player1[i, j] = minoArray[i, j];
                }
            }
        }
    }
